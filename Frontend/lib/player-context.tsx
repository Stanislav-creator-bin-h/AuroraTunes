...
interface PlayerContextType {
  ...
  listeningHistory: ListeningHistoryItem[]
  savedTracks: Track[]
  isTrackSaved: (track: Track | null) => boolean
...
const SAVED_TRACKS_KEY = "aurora_saved_tracks"
...
function loadSavedTracksFromStorage(): Track[] {
  try {
    const stored = localStorage.getItem(SAVED_TRACKS_KEY)
...
function saveSavedTracksToStorage(tracks: Track[]) {
  try {
    localStorage.setItem(SAVED_TRACKS_KEY, JSON.stringify(tracks))
  } catch (error) {
    console.error("Failed to save library tracks:", error)
  }
}
...
export function PlayerProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function restoreState() {
      try {
        ...
        setSavedTracks(loadSavedTracksFromStorage())
        ...
        if (user) {
          ...
          if (!cancelled && Array.isArray(remoteHistory)) {
  ...
  const isTrackSaved = useCallback((track: Track | null) => {
    if (!track) return false
    return savedTracks.some((savedTrack) => savedTrack.id === track.id && savedTrack.source === track.source)
  }, [savedTracks])

  const toggleSavedTrack = useCallback((track: Track) => {
    setSavedTracks((previous) => {
      const exists = previous.some((savedTrack) => savedTrack.id === track.id && savedTrack.source === track.source)
      const next = exists
        ? previous.filter((savedTrack) => !(savedTrack.id === track.id && savedTrack.source === track.source))
        : [track, ...previous]
      saveSavedTracksToStorage(next)
      return next
    })
  }, [])
