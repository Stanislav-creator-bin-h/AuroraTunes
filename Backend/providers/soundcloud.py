...
class SoundCloudProvider(BaseProvider):
    ...
    def __init__(self) -> None:
        self.client_id = os.getenv("SOUNDCLOUD_CLIENT_ID") or self._fetch_public_client_id()

    def _fetch_public_client_id(self) -> str | None:
        if requests is None:
            print("requests is not installed. SoundCloud public client_id lookup is unavailable.")
            return None

        try:
            html = requests.get("https://soundcloud.com/", timeout=10).text
            js_urls = re.findall(r'src="(https://a-v2\.sndcdn\.com/assets/[^"]+\.js)"', html)

            for url in js_urls[:8]:
                js_content = requests.get(url, timeout=10).text
                match = re.search(r'client_id\s*[:=]\s*"([a-zA-Z0-9]{32})"', js_content)
                if match:
                    return match.group(1)
        except Exception as error:
            print(f"SoundCloud client_id lookup failed: {error}")
    ...
    def get_stream_url(self, track_id: str) -> str | None:
        ...
        try:
            ...
            for key in ["http_mp3_128_url", "hls_aac_160_url", "hls_aac_96_url"]:
                if streams.get(key):
                    return streams[key]
        except Exception as error:
